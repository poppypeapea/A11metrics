...
def generate_accessible_html_outputs(html_content):
    ...
    for tag in soup.find_all(True):
        # Remove aria-label, role, and other accessibility attributes
        for attr in ['aria-label', 'role', 'aria-labelledby', 'aria-describedby']:
    ...
    for tag in impaired_soup.find_all(True):
        ...
        if not any(attr in tag.attrs for attr in ['aria-label', 'role', 'aria-labelledby', 'alt']):
        ...
        tag.attrs = {k: v for k, v in tag.attrs.items() if k in ['aria-label', 'role', 'aria-labelledby', 'alt']}
