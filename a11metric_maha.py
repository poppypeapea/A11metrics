...
# add weights to the graph based on the distance between node embeddings
...
# Calculate the similarities between the initial and trained embeddings
...
# Compare the graphs based on the node embeddings
def compare_graphs(normal_graph, impaired_graph, normal_embeddings, impaired_embeddings, metric='manhattan'):
    ...
    for node in normal_graph.nodes:
        if node in impaired_graph.nodes:
            ...
            print(f"Node {node}: Normal Mean Distance = {np.mean(normal_dist)}, Impaired Mean Distance = {np.mean(impaired_dist)}") 
...
normal_file_path = os.path.join(os.path.dirname(__file__),'files','normal.html')
impaired_file_path = os.path.join(os.path.dirname(__file__),'files','impaired.html')
